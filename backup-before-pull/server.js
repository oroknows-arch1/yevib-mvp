...
function sortUbdgEvidenceByPriority(evidence = []) {
  ...
  return normalizedEvidence.sort((a, b) => {
    ...
    return String(a.evidenceText || "").localeCompare(
      String(b.evidenceText || "")
    );
  });
}
...
function runUbdgEvidenceHelperSelfTest() {
  ...
  const normalized = normalizeUbdgEvidence(messyEvidence, 10);
  const sorted = sortUbdgEvidenceByPriority(messyEvidence);
  const summary = summarizeUbdgEvidenceStrength(messyEvidence);
  const wording = getUbdgClaimWording(summary);
  ...
  console.log("SORTED BY SOURCE PRIORITY:");
  console.log(JSON.stringify(sorted, null, 2));

  console.log("EVIDENCE STRENGTH SUMMARY:");
  console.log(
    JSON.stringify(
      {
        evidenceCount: summary.evidenceCount,
        evidenceState: summary.evidenceState,
        safeClaimLevel: summary.safeClaimLevel,
        strongestSourceType: summary.strongestSourceType,
        strongestSourcePriority: summary.strongestSourcePriority,
        sourceMix: summary.sourceMix,
        confidenceMix: summary.confidenceMix,
  ...
  console.log("CLAIM WORDING:");
  console.log(JSON.stringify(wording, null, 2));
